...
定义 `FilePreviewContent` 的自定义渲染器（`CustomRenderer`）能力契约：包括渲染器的匹配、渲染、工具组声明、事件通道、可选事件白名单，以及 React 与 Vue 双端 API 的同构要求。
...
### Requirement: React 与 Vue 双端 API 同构
...
- **THEN** 元数据区 SHALL 在 opentype 解析完成后展示真实的 family / glyphCount / format 等字段
...
### Requirement: Font Renderer 元数据状态机必须区分加载中与不可用
...
### Requirement: React 与 Vue Font Renderer 必须 WOFF2 行为一致
