...
import { renderLegacyPptHtml } from '../../fp-core';
  ...
  template: `
    @if (loading()) {
      ...
      <div class="afp-w-full afp-h-full afp-overflow-auto afp-py-6 afp-px-4">
        <div class="afp-legacy-ppt-wrap" [innerHTML]="trust(html())"></div>
      </div>
    }
  `,
})
export class PptRenderer implements RendererHandle {
  ...
  private async loadPpt(): Promise<void> {
    ...
    try {
      ...
      this.html.set(await renderLegacyPptHtml(buffer));
    } catch (err) {
      console.error('Ppt 解析错误:', err);
      this.error.set(this.t('ppt.parse_failed'));
    } finally {
