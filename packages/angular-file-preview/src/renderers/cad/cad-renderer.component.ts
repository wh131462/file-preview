...
@Component({
  ...
  standalone: true,
  template: `
    <div #container class="afp-relative afp-w-full afp-h-full afp-bg-media-bg">
      @if (error()) {
...
export class CadRendererComponent implements RendererHandle, OnDestroy {
  readonly url = input.required<string>();
  readonly file = input<File | undefined>(undefined);
  ...
  private readonly wireframe = signal(false);
  private readonly showGrid = signal(true);
  private readonly showAxes = signal(true);
  ...
  private loadModel(): void {
    ...
    const loadPromise = file
      ? file.arrayBuffer()
      : this.fetcher()(this.url(), { signal: this.abortController?.signal }).then((response) => {
