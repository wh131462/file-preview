...
import { FontPreviewLineComponent } from './font-preview-line.component';
...
function detectMagic(buf: ArrayBuffer): DetectedFormat {
...
@Component({
  ...
  standalone: true,
  imports: [FontPreviewLineComponent, RendererErrorComponent],
  template: `
    <div class="afp-flex afp-flex-col afp-w-full afp-h-full afp-overflow-hidden">
      @if (loading()) {
        <div class="afp-flex afp-items-center afp-justify-center afp-w-full afp-h-full">
          <div class="afp-text-fg-secondary">{{ translator.t()('font.loading') }}</div>
        </div>
      } @else if (error() || !metadata()) {
        <afp-renderer-error [message]="error() || translator.t()('font.parse_failed')" />
      } @else if (metadata(); as meta) {
        <div class="afp-flex-shrink-0 afp-px-6 afp-py-4 afp-bg-surface-1 afp-border-b afp-border-line-weak afp-min-w-0">
          <div class="afp-grid afp-grid-cols-2 afp-gap-x-6 afp-gap-y-2 afp-text-sm afp-min-w-0">
            <div class="afp-flex afp-items-center afp-gap-2 afp-min-w-0">
              <span class="afp-text-fg-tertiary afp-flex-shrink-0">{{ translator.t()('font.meta.family') }}:</span>
              <span class="afp-text-fg-primary afp-font-semibold afp-truncate">
                {{ showMetaPlaceholder() ? metaPlaceholder() : meta.family }}
              </span>
            </div>
            @if (meta.subfamily) {
              <div class="afp-flex afp-items-center afp-gap-2 afp-min-w-0">
                <span class="afp-text-fg-tertiary afp-flex-shrink-0">{{ translator.t()('font.meta.subfamily') }}:</span>
                <span class="afp-text-fg-primary afp-truncate">{{ meta.subfamily }}</span>
              </div>
            }
            <div class="afp-flex afp-items-center afp-gap-2 afp-min-w-0">
              <span class="afp-text-fg-tertiary afp-flex-shrink-0">{{ translator.t()('font.meta.format') }}:</span>
              <span class="afp-text-fg-primary afp-truncate">{{ meta.format }}</span>
            </div>
            <div class="afp-flex afp-items-center afp-gap-2 afp-min-w-0">
              <span class="afp-text-fg-tertiary afp-flex-shrink-0">{{ translator.t()('font.meta.glyphs') }}:</span>
              <span class="afp-text-fg-primary afp-truncate">
                {{ showMetaPlaceholder() ? metaPlaceholder() : meta.glyphCount }}
              </span>
            </div>
            @if (meta.designer) {
              <div class="afp-flex afp-items-center afp-gap-2 afp-min-w-0">
                <span class="afp-text-fg-tertiary afp-flex-shrink-0">{{ translator.t()('font.meta.designer') }}:</span>
                <span class="afp-text-fg-primary afp-truncate">{{ meta.designer }}</span>
              </div>
            }
            @if (meta.version) {
              <div class="afp-flex afp-items-center afp-gap-2 afp-min-w-0">
                <span class="afp-text-fg-tertiary afp-flex-shrink-0">{{ translator.t()('font.meta.version') }}:</span>
                <span class="afp-text-fg-primary afp-truncate">{{ meta.version }}</span>
              </div>
            }
          </div>
        </div>
          ...
          <div class="afp-px-6 afp-py-6 afp-space-y-8 afp-min-w-0">
            <div class="afp-min-w-0">
              <label class="afp-block afp-text-xs afp-text-fg-tertiary afp-mb-2">
                {{ translator.t()('font.sample_text_placeholder') }}
              </label>
              <textarea
                ...
                rows="2"
                [placeholder]="translator.t()('font.sample_text_placeholder')"
...
export class FontRendererComponent implements RendererHandle {
  readonly url = input.required<string>();

  readonly translator = injectTranslator();
  private readonly fetcher = injectFetcher();
  readonly resolvedTheme = injectResolvedTheme();

  readonly sizes = SIZES;
  readonly samples = DEFAULT_SAMPLES;
  ...
  readonly metaPlaceholder = computed(() =>
    this.metadataStatus() === 'loading'
      ? this.translator.t()('font.metadata_loading')
      : this.translator.t()('font.metadata_unavailable'),
  );
  ...
  constructor() {
    effect(() => {
      const url = this.url();
      void this.loadFont(url);
    });

    this.destroyRef.onDestroy(() => this.clearFontFace());
  }
  ...
  onCustomTextInput(e: Event) {
    this.customText.set((e.target as HTMLTextAreaElement).value);
  }
  ...
  private async loadMetadata(arrayBuffer: ArrayBuffer, magic: DetectedFormat, url: string): Promise<void> {
    if (magic === 'woff2') {
    ...
    this.metadata.set({
      ...
      format: this.formatLabel(magic, url),
    });
  ...
  private async loadFont(url: string) {
    ...
    try {
      ...
      const metadataPromise = this.loadMetadata(arrayBuffer, magic, url);
      ...
      this.error.set(this.translator.t()('font.load_failed'));
      this.loading.set(false);
      this.metadataStatus.set('unavailable');
    }
  }

  private clearFontFace() {
    if (this.fontFace) {
