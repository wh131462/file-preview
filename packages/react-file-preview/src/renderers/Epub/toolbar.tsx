...
export function getEpubToolbarGroups(ctx: EpubToolbarContext): ToolbarGroup[] {
  return [
    {
      items: [
        { type: 'button', icon: <List className="rfp-w-4 rfp-h-4" />, tooltip: ctx.t('toolbar.toc'), action: () => ctx.epubRef.current?.toggleToc() },
      ],
    },
    {
      items: [
        { type: 'button', icon: <ChevronLeft className="rfp-w-4 rfp-h-4" />, tooltip: ctx.t('toolbar.prev_page'), action: () => ctx.epubRef.current?.prevPage() },
        { type: 'text', content: `${ctx.current} / ${ctx.total}`, minWidth: '4rem' },
        { type: 'button', icon: <ChevronRight className="rfp-w-4 rfp-h-4" />, tooltip: ctx.t('toolbar.next_page'), action: () => ctx.epubRef.current?.nextPage() },
      ],
    },
    {
      items: [
