import { ZoomIn, ZoomOut, RefreshCw, ChevronLeft, ChevronRight, List } from 'lucide-vue-next';
...
export function getPdfToolbarGroups(ctx: PdfToolbarContext): ToolbarGroup[] {
  ...
  if (ctx.onToggleOutline) {
    groups.push({
      items: [
        { type: 'button', icon: List, tooltip: ctx.t('toolbar.outline'), action: ctx.onToggleOutline },
      ],
    });
  }
  ...
  groups.push({
    items: [
      { type: 'button', icon: ChevronLeft, tooltip: ctx.t('toolbar.prev_page'), action: ctx.onPrevPage, disabled: ctx.currentPage <= 1 },
      { type: 'text', content: `${ctx.currentPage} / ${ctx.totalPages}`, minWidth: '4rem' },
      { type: 'button', icon: ChevronRight, tooltip: ctx.t('toolbar.next_page'), action: ctx.onNextPage, disabled: ctx.currentPage >= ctx.totalPages },
    ],
  });
